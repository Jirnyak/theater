# 🛠️ Contributing to Jirnyak/theater
...
> Technology Foundation: `JavaScript / Canvas 2D / Web Audio`
...
Theater Scenario Transition & Finite State Machine Engine is engineered for maximum performance, deterministic state transitions, and zero computational slop. All contributions must respect existing subsystem boundaries and data flows:

```mermaid
graph TD
    A[Player Interaction Event] --> B[Finite State Machine Core]
    B -->|Guard Condition Validated| C[Scene Transition Dispatcher]
    C -->|Crossfade Audio & Mesh| D[Active Gameplay Viewport]
```
...
* **Formal Requirement**: FSM transitions must follow guard condition evaluations with zero ambiguous states.
...
### 2. Bidirectional Maze Guarantees
* **Formal Requirement**: Maze generation algorithms must ensure exit paths are always traversable.
* **Verification Protocol**: Automated unit test assertion + mathematical boundary check.
* **Failure Mode**: Immediate build rejection; PR cannot be approved without meeting this invariant.
...
* Primary Runtime: `JavaScript / Canvas 2D / Web Audio`
...
```typescript
// ✅ CORRECT: Guarded FSM Transition
function transition(fsm: FSM, event: string): boolean {
    const nextState = fsm.transitions[fsm.currentState]?.[event];
    if (nextState && nextState.guard(fsm.context)) {
        fsm.currentState = nextState.target;
        return true;
    }
    return false;
}
